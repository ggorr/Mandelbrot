...
const filtrate = (data, width, filter = 'none', bpp = 4) => {
...
const filtrateMinSum = (data, width, bpp = 4) => {
...
const filtrateLineFull = (data, width, bpp = 4) => {
...
const filtrateNoneLine = (dst, src, bpp) => {
    dst[0] = 0;
    dst.set(src.slice(src.length >>> 1, src.length), 1);
};
// bpp: bytes(samples) per pixel
const filtrateSubLine = (dst, src, bpp) => {
...
// bpp: bytes(samples) per pixel
const filtrateUpLine = (dst, src, bpp) => {
    const lineBytes = src.length >>> 1;
    dst[0] = 2;
...
// bpp: bytes(samples) per pixel
const filtrateAverageLine = (dst, src, bpp) => {
...
// bpp: bytes(samples) per pixel
const filtratePaethLine = (dst, src, bpp) => {
    let lineBytes = src.length >>> 1;
    dst[0] = 4;
    let srcPos = lineBytes;
    let srcUp = 0; // srcUp = srcPos - bpp * width
...
// bpp: bytes(samples) per pixel
const filtrateNoneFull = (src, width, bpp) => {
...
// bpp: bytes(samples) per pixel
const filtrateSubFull = (src, width, bpp) => {
    const lineBytes = bpp * width;
    const height = src.length / lineBytes;
    const dst = new Uint8Array((lineBytes + 1) * height);
    let srcPos = 0;
...
// bpp: bytes(samples) per pixel
const filtrateUpFull = (src, width, bpp) => {
    const lineBytes = bpp * width;
    const height = src.length / lineBytes;
    const dst = new Uint8Array((lineBytes + 1) * height);
    dst[0] = 2;
...
// bpp: bytes(samples) per pixel
const filtrateAverageFull = (src, width, bpp) => {
    const lineBytes = bpp * width;
    const height = src.length / lineBytes;
    const dst = new Uint8Array((lineBytes + 1) * height);
    dst[0] = 3;
...
// bpp: bytes(samples) per pixel
const filtratePaethFull = (src, width, bpp) => {
    const lineBytes = bpp * width;
    const height = src.length / lineBytes;
    const dst = new Uint8Array((lineBytes + 1) * height);
    dst[0] = 4;
